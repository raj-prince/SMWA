...
public class ResponseDaoJpaImpl implements ResponseDao {
	...
	public List<Survey> getSurvey(int username) {
		TypedQuery<Survey> query = em.createQuery("select s from Survey s where s.surveyId in "
				+ "(select d.surveyId from Distribution d where d.surveyStatus='0' and d.userId="+"'"+username +"'"+")", Survey.class);
	...
	public List<Survey> getClosedSurvey(int username) {
		TypedQuery<Survey> query = em.createQuery("select s from Survey s where s.surveyId in "
				+ "(select d.surveyId from Distribution d where d.surveyStatus='1' and  d.userId="+username+")", Survey.class);
		return query.getResultList();
	}
	...
	public List<Response> getResponse(int qid, int userid) {
		...
		TypedQuery<Response> query = em.createQuery("select r from Response r "
				+ "where r.questionId="+qid +"and r.userId="+userid,Response.class);
		return query.getResultList();
	}

	@Override
	public void updateDistributionTable(int sid, int uname) {
		Query query = em.createQuery("update Distribution d set "
				+ "d.surveyStatus='1' where d.userId="+"'"+uname+"'" +"and d.surveyId="+sid);
		query.executeUpdate();
	}
	
	@Override
	public long count(int questionId, String ans) {
		TypedQuery<Long> query = em.createQuery("select count(*) from Response res "
				+ "where res.questionId="+questionId+" "+ "and res.responseText="+"'" +ans+"'", Long.class);
		return query.getResultList().size() == 0 ? 0 : query.getResultList().get(0);
	}
	
	@Override
	public List<String> getResponse(int questionId) {
		TypedQuery<String> query = em.createQuery("select res.responseText from Response res "
				+ "where res.questionId="+questionId, String.class);
		return query.getResultList();
	}
